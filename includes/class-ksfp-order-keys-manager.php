...
class Ksfp_Order_Keys_Manager {
	...
	private function get_key_for_product( $product_id ) {
		...
		if ( ! empty( $key_posts ) ) {
			...
			update_post_meta( $key_post->ID, 'ksfp_game_key_status', 'inactive' );

			$key = get_post_meta( $key_post->ID, 'ksfp_game_key', true );
			return $key;
		}
	...
	private function send_key_email( $order, $item, $key ) {
		...
		$game_key                  = get_post_meta( $key_post_id, 'ksfp_game_key', true );
